...
public class FinalOrderController {
    ...
    // ─────────────────────────────────────────────────────────────────
    //  Helper: per-size wholesale price from ProductInventory
    //  Returns the wholesalePrice stored on the SizeQuantity object
    //  for the requested size label.  Falls back to fabric wholesale
    //  only when the size entry is missing or its price is 0.
    // ─────────────────────────────────────────────────────────────────
    private double getSizeWholesalePrice(ProductInventory inventory, String sizeLabel) {
        if (inventory.getSizes() != null) {
            for (ProductInventory.SizeQuantity sq : inventory.getSizes()) {
                if (sizeLabel.equals(sq.getLabel()) && sq.getWholesalePrice() > 0) {
                    return sq.getWholesalePrice();
                }
            }
        }
        // Fall back to fabric-level wholesale price
        Fabric fabric = inventory.getFabric();
        return (fabric != null) ? fabric.getWholesalePrice() : 0.0;
    }

    // ─────────────────────────────────────────────────────────────────
    //  Helper: resolve effective price for a product size
    //  Priority: 1) user-specific discount  2) per-size wholesale price
    //            3) fabric-level wholesale price
    // ─────────────────────────────────────────────────────────────────
    private double resolveProductPrice(ProductInventory inventory,
                                       String sizeLabel,
                                       List<Discount> userDiscounts) {
        // 1. Check user discount (discount is keyed to productInventoryId, not size)
        if (userDiscounts != null && !userDiscounts.isEmpty()) {
            Double discounted = findDiscountedPriceForProduct(inventory.getId(), userDiscounts);
            if (discounted != null && discounted > 0) {
                return discounted;
            }
        }
        // 2. Per-size wholesale price (falls back to fabric wholesale internally)
        return getSizeWholesalePrice(inventory, sizeLabel);
    }

    // ─────────────────────────────────────────────────────────────────
    //  Helper: resolve effective price for a product set
    //  Sets don't have per-size pricing; we use fabric wholesale price
    //  unless a user discount exists.
    // ─────────────────────────────────────────────────────────────────
    private double resolveSetPrice(ProductSets productSet, List<Discount> userDiscounts) {
        if (userDiscounts != null && !userDiscounts.isEmpty()) {
            Double discounted = findDiscountedPriceForProduct(productSet.getId(), userDiscounts);
            if (discounted != null && discounted > 0) {
                return discounted;
            }
        }
    ...
    // ─────────────────────────────────────────────────────────────────
    //  POST /api/finalorder/recalculate-order-prices
    // ─────────────────────────────────────────────────────────────────
    @PostMapping("/recalculate-order-prices")
    public ResponseEntity<String> recalculateOrderPrices(
            @RequestHeader("Authorization") String tokenHeader,
        ...
        try {
            ...
            for (ProductOrder.ProductEntry productEntry : order.getProductEntries()) {
                ...
                for (ProductOrder.OrderedSizeQuantity orderedSize : productEntry.getOrderedSizes()) {
                    // Resolve price per size (uses per-size wholesale price)
                    double effectivePrice = resolveProductPrice(inventory, orderedSize.getLabel(), userDiscounts);

                    double quantity = orderedSize.getQuantity();
                    productEntryTotal += effectivePrice * quantity;

                    recalculatedSizes.add(new ProductOrder.OrderedSizeQuantity(
                            orderedSize.getLabel(),
                            orderedSize.getQuantity(),
                            effectivePrice
                    ));
                ...
                recalculatedTotalAmount += productEntryTotal;
                recalculatedProductEntries.add(new ProductOrder.ProductEntry(inventory, recalculatedSizes));
            }
            ...
            for (ProductOrder.OrderedSets orderedSet : order.getSets()) {
                ...
                double effectivePrice = resolveSetPrice(productSet, userDiscounts);
                ...
                double setTotal = effectivePrice * totalSetQuantity * orderedSet.getQuantity();
                recalculatedTotalAmount += setTotal;

                recalculatedSets.add(new ProductOrder.OrderedSets(
                        productSet,
                        orderedSet.getQuantity(),
                        effectivePrice
                ));
    ...
    // ─────────────────────────────────────────────────────────────────
    //  POST /api/finalorder/final-order
    // ─────────────────────────────────────────────────────────────────
    @PostMapping("/final-order")
    public ResponseEntity<String> placeOrder(
            @RequestHeader("Authorization") String tokenHeader,
        ...
        try {
            ...
            for (Cart.CartProductItem item : cart.getProducts()) {
                ...
                for (Cart.CartProductItem.SizeQuantity selectedSize : item.getSelectedSizes()) {
                        ...
                        if (availableSize.getLabel().equals(selectedSize.getLabel())) {
                            ...
                            if (availableSize.getQuantity() < selectedSize.getQuantity()) {
                                return ResponseEntity.badRequest().body(
                                        "Error: Not enough quantity for size "
                                                + selectedSize.getLabel() + " in product ID: " + inventory.getId());
                            }
                            break;
                        }
                    ...
                    if (!sizeFound) {
                        return ResponseEntity.badRequest().body(
                                "Error: Size " + selectedSize.getLabel()
                                        + " not available in product ID: " + inventory.getId());
                    }
                }
            }

            // ── Validate set quantities ───────────────────────────────
            for (Cart.CartSetItem setItem : cart.getSets()) {
                ProductSets productSet = setItem.getProductSet();
                if (setItem.getQuantity() > productSet.getTotalQuantity()) {
                    return ResponseEntity.badRequest().body(
                            "Error: Not enough quantity in set " + productSet.getSetName()
                                    + " (ID: " + productSet.getId() + ")");
                }
            }

            // ── Build product entries with per-size pricing ───────────
            double totalAmount = 0.0;
            List<ProductOrder.ProductEntry> productEntries = new ArrayList<>();
            ...
            for (Cart.CartProductItem item : cart.getProducts()) {
                ...
                for (Cart.CartProductItem.SizeQuantity size : item.getSelectedSizes()) {
                    // Resolve price: discount → per-size wholesale → fabric wholesale
                    double effectivePrice = resolveProductPrice(inventory, size.getLabel(), userDiscounts);
                    double qty = size.getQuantity();

                    System.out.printf("Product %s  size %s  price %.2f  qty %.0f%n",
                            inventory.getId(), size.getLabel(), effectivePrice, qty);

                    itemTotal += effectivePrice * qty;
                    orderedSizes.add(new ProductOrder.OrderedSizeQuantity(
                            size.getLabel(), (int) qty, effectivePrice));
                }
            ...
            for (Cart.CartSetItem setItem : cart.getSets()) {
                ...
                // Total pieces inside one set
                int piecesPerSet = 0;
                if (productSet.getSizes() != null) {
                    for (ProductSets.SizeQuantity size : productSet.getSizes()) {
                        piecesPerSet += size.getQuantity();
                    }
                }

                double effectivePrice = resolveSetPrice(productSet, userDiscounts);

                System.out.printf("Set %s  price %.2f  piecesPerSet %d  qty %d%n",
                        productSet.getId(), effectivePrice, piecesPerSet, setItem.getQuantity());

                double setTotal = effectivePrice * piecesPerSet * setItem.getQuantity();
                totalAmount += setTotal;
                orderedSets.add(new ProductOrder.OrderedSets(
                        productSet, setItem.getQuantity(), effectivePrice));
            }
            ...
            order.setApproved(admin != null && admin.equalsIgnoreCase("admin") ? "approved" : "Pending");
            order.setOrderDate(LocalDateTime.now());

            order = productOrderRepository.save(order);

            // ── Deduct inventory: products ────────────────────────────
            for (Cart.CartProductItem item : cart.getProducts()) {
                ProductInventory inventory = item.getProductInventory();
                List<ProductInventory.SizeQuantity> updatedSizes = new ArrayList<>();
            ...
            // ── Deduct inventory: sets ────────────────────────────────
            for (Cart.CartSetItem setItem : cart.getSets()) {
                ProductSets productSet = setItem.getProductSet();
                productSet.setTotalQuantity(productSet.getTotalQuantity() - setItem.getQuantity());
                productSetsRepository.save(productSet);
            }

            cartRepository.delete(cart);

            // ── If admin order, record khata transaction ──────────────
            if (admin != null && admin.equalsIgnoreCase("admin")) {
                KhataBookRequestDTO dto = new KhataBookRequestDTO();
                dto.setUserId(order.getUser().getUserId());
    ...
    // ─────────────────────────────────────────────────────────────────
    //  Helper: find user-specific discounted price for a product/set id
    // ─────────────────────────────────────────────────────────────────
    private Double findDiscountedPriceForProduct(String productId, List<Discount> userDiscounts) {
        for (Discount discount : userDiscounts) {
            if (discount.getDiscountWithProducts() != null) {
                for (Discount.DiscountWithProduct dp : discount.getDiscountWithProducts()) {
                    if (productId.equals(dp.getProductId())) {
                        return dp.getPrice();
                    }
                }
            }
    ...
    // ─────────────────────────────────────────────────────────────────
    //  POST /api/finalorder/disapprove-order
    // ─────────────────────────────────────────────────────────────────
    @PostMapping("/disapprove-order")
    public ResponseEntity<String> disapproveOrder(
            @RequestHeader("Authorization") String tokenHeader,
        ...
        try {
            ...
            // Restore set inventory
            for (ProductOrder.OrderedSets orderedSet : order.getSets()) {
                ProductSets productSet = orderedSet.getProductSet();
                productSet.setTotalQuantity(productSet.getTotalQuantity() + orderedSet.getQuantity());
                productSetsRepository.save(productSet);
            }
